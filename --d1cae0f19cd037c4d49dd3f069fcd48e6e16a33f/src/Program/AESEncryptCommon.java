...
AESEncryptCommon extends ProgCommon {

	final static int Nb = 4;		// words in a block, always 4 for now 分组大小
	static int Nk;			// key length in words 密钥长度

	static int Nr;			// number of rounds, = Nk + 6 加密轮数
	static short[] w;			// the expanded key 扩展密钥数组

	protected static int[][] STable;
	static LookupTableAgent agent;

	private static Mulin ccMUL;
	...
	static void initCircuits() {
		ccsin = new Circuit[1];
		ccsin[0] = ccMUL = new Mulin(4);//multi input
		ccs = new Circuit[3];
		ccs[0] = ccARK = new AddRoundKey();//轮密钥
		ccs[1] = ccSBX = new SBox();//S盒
		ccs[2] = ccMXC = new MixColumns();//列混淆
	}
	...
	public static State Cipher(State key, State msg) {
		State[] arrS;
//		System.out.println("mulin-fore");
		State statein = Mulin(msg);
//		System.out.println("mulin-end");

		State state = AddRoundKey(key, statein, 0);
	...
	// Multi input
	private static State Mulin(State state){
		return ccMUL.startExecuting(state);
	}
